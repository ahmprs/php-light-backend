...
// narrate a given number to persian text
function nar($num)
{
    ...
    for ($i = 0; $i < $n; $i++) {
        $n3 = nar3($ts[$i]);
        if (strcmp($n3['str'], '') == 0) continue;

        if (count($arr) > 0) $arr[] = $and_char;
        $arr[] = $n3['str'];
        $m = count($n3['arr']);
        $t = '';
        for ($j = 0; $j < $m; $j++) {
        ...
        if (strcmp($t, '') != 0) {
            $prfx = $Pr[$n - $i - 1];
            if (strcmp(trim($prfx), '') == 0) continue;
            $arr[] = $prfx;
        }
    }
